...
const HikingPage = () => {
    ...
    const [selectedHiking, setSelectedHiking] = useState(null);
                ...
                if (hikingData.length > 0) {
                    setAllHiking(
                        hikingData.map((hiking) => ({
                            ...
                            itinarary: hiking.itinarary || [],
                            included: hiking.included || [],
                            excluded: hiking.excluded || [],
                            description:
                                hiking.description ||
                                "No description available.",
                            what_to_bring:
                                hiking.what_to_bring ||
                                "No recommendations provided.",
                            duration: hiking.duration || "N/A",
                            elevation: hiking.elevation || "N/A",
                            season: hiking.season || "N/A",
                            best_time_to_visit:
                                hiking.best_time_to_visit || "N/A",
                            title: hiking.title || "Unnamed Hike",
                        }))
                    );
                    setSelectedHiking(hikingData[0]);
                } else {
                    setError("No hiking data found");
                }
            } catch (err) {
                console.error("Error fetching hiking data:", err);
    ...
    if (loading) {
        return (
            <div className="container mx-auto flex justify-center items-center h-64">
                <div className="text-xl">Loading hiking details...</div>
            </div>
        );
    }
    ...
    if (!selectedHiking) {
        return (
            <div className="container mx-auto flex justify-center items-center h-64">
                <div className="text-xl">No hiking data available</div>
            </div>
        );
    }
            ...
            <div
                ...
                style={{
                    backgroundImage: `url('${imgurl}/${
                        selectedHiking?.images[0]?.image_path || ""
                    }')`,
                }}
            >
            ...
            <div className="flex flex-col lg:flex-row gap-8 my-8 lg:px-10">
                ...
                <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        {selectedHiking.images.length > 0 && (
                            <>
                                <div className="md:col-span-2">
                                    <img
                                        src={`${imgurl}/${selectedHiking.images[0]?.image_path}`}
                                        alt="Main View"
                                        className="w-full h-64 md:h-80 object-cover rounded-2xl shadow-md"
                                    />
                                </div>
                                <div className="grid grid-cols-1 gap-4">
                                    {selectedHiking.images
                                        .slice(1, 3)
                                        .map((image, index) => (
                                            <img
                                                key={index}
                                                src={`${imgurl}/${image.image_path}`}
                                                alt={`Hiking view ${index + 1}`}
                            ...
                            <div className="py-6">
                                ...
                                {openIndex === 1 && (
                                    <div className="space-y-6 mt-4">
                                        {selectedHiking.itineraries.length >
                                        0 ? (
                                            selectedHiking.itineraries.map(
                                                (day, index) => (
                                                    <div
                                                        key={index}
                                                        className="flex"
                ...
                <div className="w-full lg:w-[350px] lg:sticky lg:top-24 h-fit border border-gray-200 rounded-xl shadow-md p-6 bg-white">
                    ...
                    <div className="space-y-5">
                        ...
                        <DetailItem
                            ...
                            bgColor="bg-blue-100"
                            label="Max Elevation"
                            value={selectedHiking.elevation}
                        />
                        ...
                        <DetailItem
                            ...
                            value={selectedHiking.best_time_to_visit}
                        />
                    </div>
                </div>
            </div>
